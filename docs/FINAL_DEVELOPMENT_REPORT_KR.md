...
작성일: 2026-03-15 (최초: 2026-02-21 → v2.0 업데이트 → v3.0 릴리즈 2026-03-22 → v3.1 업데이트 2026-04-08)
...
4. 2026-03-22 기준: dataset **30일 확보 완료**, 631 샘플, 방향정확도 **0.894**, 보상 **+0.0064**, 게이트 **7/7 전체 통과**.
5. **완료**: 릴리즈 태그 `release-agent-20260322-105138` 생성 및 원격 저장소 푸시. 이후 7일 주기 자동 재학습 운영 중.
6. **2026-04-08 기준**: dataset **47일 확보**, 302 샘플(롤링 윈도우), 방향정확도 **0.891**, 보상 **+0.00172**. 파라미터 적극 튜닝(MCTS 60%, 엔트로피 0.45, 2000 스텝) 후 재학습 완료. 현재 abstain 게이트(93.38% > 93%) 실패로 BLOCKED 상태. 2일 추가 데이터 누적(5/7일) 후 자동 재학습 예정.
...
- 기능:
  ...
  - **4-신호 블렌드**: `0.60×MCTS + 0.20×보상정책 + 0.10×f-net prior + 0.10×ActionModel prior` (적극 튜닝: MCTS 가중치 상향)
  - `safe_mode` 게이트:
    - 신뢰도 낮음(`confidence < 0.25`) → 보수적 `HOLD` 전환
    - 불확실성 높음(`entropy > 1.58`) → 보수적 `HOLD` 전환
  - **Anti-collapse**: entropy < 0.45 시 균등 분포(uniform)와 혼합 (임계값 0.65→0.45로 낮춤, 더 자신감 있는 결정 허용)
...
- 게이트 (7개):
  1. `accuracy_raw >= 0.55`
  2. `avg_reward_raw > 0`
  3. `abstain_ratio <= 0.93` (2026-04-03 조정: 0.85→0.90→0.93)
...
| 지표 | 2026-02-21 | 2026-03-15 | 2026-03-22 | **2026-04-08** |
|------|-----------|-----------|-----------|--------------|
| 데이터 | 14일 | 22일 | 30일 | **47일** |
| 샘플 수 | 42 | 308 | 631 | **302** |
| 방향정확도 | 0.619 | **0.630** | **0.894** | **0.891** |
| 평균 보상 | -0.000275 | **+0.000276** | **+0.0064** | **+0.00172** |
| 관망비율 | 0.667 | 0.659 | 0.788 | **0.934** |
| 평균 신뢰도 | 0.228 | 0.231 | 0.335 | **0.373** |
| 게이트 통과 | 5/7 | **6/7** | **7/7** | **6/7 (BLOCKED)** |

> **Note (2026-04-08)**: 샘플 수 감소(631→302)는 롤링 윈도우 방식으로 평가 대상 기간이 조정된 결과이며 데이터 손실이 아닙니다. 관망비율 급등(78.8%→93.4%)은 MCTS 60% 가중치 적용 후 에이전트가 불확실한 시장 조건에서 더 보수적으로 행동하는 경향을 반영합니다.
...
1. 데이터 윈도우: 30일 이상 (현재: **47일 달성** ✓)
2. 정확도 게이트(raw): `directional_accuracy_buy_vs_wait_raw >= 0.55` ✓ (0.891)
3. 보상 게이트(raw): `avg_reward_per_decision_raw > 0` ✓ (+0.00172)
4. 관망비율 게이트: `abstain_ratio <= 0.93` ❌ (93.38% — 0.38% 초과)
...
**현재 판정: BLOCKED** — abstain 게이트 1개 실패. 7일 주기 자동 재학습(잔여 2일)에서 해소 예정.

## 6. 운영 상태 이력

### 2026-03-22 (v3.0 릴리즈)
- ✅ **30일 데이터 윈도우**: 30일 달성 (2026-02-21 ~ 2026-03-22)
- ✅ **uplift_raw_vs_buy**: +0.0040 (30일 실데이터 학습 후 통과)
- ✅ **릴리즈 태그**: `release-agent-20260322-105138` 생성 및 푸시
- ✅ **Docker 배포**: backend:8000 + frontend:3000 정상 서비스 확인

### 2026-04-03 (v3.1 파라미터 튜닝)
- ✅ **데이터 확장**: 42일 룩백으로 재학습
- ✅ **파라미터 튜닝**: MCTS 60%, 엔트로피 임계 0.45, 2000 스텝 적용
- ✅ **재학습 완료**: `alphazero_model_agent_latest.pth` 업데이트
- ⚠️ **게이트 상태**: abstain 93.38% > 93% 임계로 BLOCKED

### 2026-04-08 (현재)
- 데이터: **47일** 누적 (2026-02-21 ~ 2026-04-08)
- 마지막 학습: 2026-04-03 (5일 전), 누적 신규 데이터 5일/7일
- 다음 자동 재학습 예상: 2026-04-10 (7일 간격, 2일 후)
- 파이프라인 상태: **BLOCKED** (abstain 게이트)
...
**실증 완성도**: ✅ 최초 완성 (2026-03-22). 현재 운영 중.
- 최초 릴리즈: 7/7 게이트 PASS (2026-03-22)
- 현재 파라미터 튜닝 후 재학습 중 — abstain 게이트 0.38% 초과로 BLOCKED
- 7일 주기 자동 재학습으로 2026-04-10경 해소 예상

**최초 완성 시점**: 2026-03-22 (릴리즈 태그 `release-agent-20260322-105138`)
**현재 상태 (2026-04-08)**: BLOCKED — 47일 데이터, 적극 파라미터 튜닝 적용, 다음 재학습 2일 후 예정
