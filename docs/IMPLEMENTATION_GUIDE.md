...
1.  **네트워크 정의 (`backend/models/`)**:
    *   `representation_network.py`: 입력을 AI가 이해하는 벡터로 변환. 잔차 연결(residual connection)로 그래디언트 소실을 방지.
    *   `dynamics_network.py`: 가상 세계의 규칙(물리학)을 학습. 보상 불확실성(log-variance)도 함께 출력.
    *   `prediction_network.py`: 최선의 선택과 그 가치를 판단.
    *   `action_model.py`: 잠재 상태를 기반으로 행동의 사전 확률을 예측. 정책 보정의 4번째 신호로 사용됨.
2.  **MCTS 알고리즘 (`mcts.py`)**:
