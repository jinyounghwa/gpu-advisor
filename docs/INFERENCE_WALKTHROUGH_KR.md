...
```
입력:  state_vector = [0.234, 0.228, ...] (256D — feature_engineer 출력)
       ↓
       Linear(256, 256) + LayerNorm
       ↓
       잔차 연결 포함 FeedForward × 3 (x = x + block(x))
...
**핵심**: 256차원의 GPU 시장 상태 벡터(가격 추세, 환율, 뉴스 감성, 기술 지표)를 계획에 최적화된 256차원 잠재 표현으로 재인코딩합니다. 네트워크는 원시 피처를 MCTS 시뮬레이션에 더 적합한 잠재 공간으로 압축 및 재가중합니다.
...
사전 확률(f-net): [0.20, 0.25, 0.22, 0.18, 0.15]  (데이터 분포)
ActionModel prior:[0.10, 0.28, 0.30, 0.17, 0.15]  (잠재 상태 조건부 학습된 사전 확률)
