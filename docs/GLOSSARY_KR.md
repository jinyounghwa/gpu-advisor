...
### Action Model (행동 모델, a)
잠재 상태를 조건으로 행동의 사전 확률을 출력하는 신경망 모듈. ActionEmbeddingLayer(5개 행동 → 16D 의미 임베딩)와 ActionPriorNetwork(256D → 5개 softmax 확률)로 구성. 정책 보정의 4번째 신호로 사용되며, 기존 하드코딩된 효용 편향(utility_bias) 휴리스틱을 대체. 약 42K 파라미터.
...
### Calibration (보정)
MCTS 정책을 다른 신호와 혼합하여 최종 행동 확률을 조정하는 과정. 공식: 0.45×MCTS + 0.25×보상 + 0.15×f-net Prior + 0.15×ActionModel Prior. 4번째 신호(ActionModel)가 기존 하드코딩된 효용 편향 휴리스틱을 대체.

### Checkpoint (체크포인트)
학습된 모델의 가중치를 저장한 파일. `.pth` 확장자. h, g, f 네트워크와 선택적으로 ActionModel(a)의 state_dict 및 메타데이터 포함. 하위 호환성: `a_state_dict`가 없는 체크포인트는 효용 편향 휴리스틱으로 폴백.
...
### PUCT (Predictor + UCT)
AlphaZero/MuZero 방식 MCTS에서 사용하는 UCB 변형. 공식: Q + c·P·√N_parent / (1+N_child). c는 탐험 상수(이 프로젝트에서 √2), P는 신경망 사전확률. 표준 UCB1이 ln(N)을 사용하는 것과 달리 √N을 사용하여 초기 탐험이 더 빠름.
...
블록의 입력을 출력에 더하는 지름길 연결: `x = x + block(x)`. 깊은 신경망에서 그래디언트 소실을 방지하며, 최소한 항등 매핑(identity mapping)이 보존됨을 보장. 세계 모델 3개 네트워크(h, g, f) 모두에 적용.
...
탐험-활용 균형을 위한 점수. 이 프로젝트에서는 PUCT 변형을 사용: Q + c·P·√N_parent / (1+N). 미방문 노드는 UCB = ∞로 초기 탐험을 보장. 참고: PUCT.
...
### World Model (세계 모델)
환경의 동작을 시뮬레이션하는 학습된 모델. 이 프로젝트에서 Representation(h) + Dynamics(g) + Prediction(f) 세 네트워크로 구성. Action Model(a)은 이 삼중구조를 보완하여 잠재 상태 기반 행동 사전 확률을 정책 보정에 제공.
