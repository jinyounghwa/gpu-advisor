...
- **`auto_training.py`**: 30일 도달 여부와 누적 데이터 간격을 보고 자동으로 `드라이 체크` 또는 `학습+릴리즈`를 실행하는 정책 엔진입니다. 상태 파일(`data/processed/auto_training_state.json`)을 유지해 재학습 주기를 관리합니다.
- **`run_daily.py`**: 위 모듈들을 순차적으로 가동시키는 일일 오케스트레이터입니다. 기본 실행에서 자동 학습 오케스트레이션까지 수행하며, 필요 시 `--disable-auto-train`으로 드라이 체크만 실행할 수 있습니다. 결과를 `docs/reports/YYYY-MM-DD/*.json|*.md`로 자동 기록합니다.
...
## 🛠️ 6. 루트 스크립트 및 문서
- **`setup_cron.sh`**: 매일 자정에 `run_daily.py`가 실행되도록 시스템 스케줄러를 자동 설정합니다. 30일 이후에는 자동 학습/재학습 루틴까지 같은 배치에서 수행됩니다.
